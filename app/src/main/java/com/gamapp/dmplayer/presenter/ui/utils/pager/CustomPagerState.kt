...
//import com.google.accompanist.pager.DebugLog
...
/**
 * Creates a [CustomPagerState] that is remembered across compositions.
 *
 ...
 * been created. Changes to [pageCount] will result in the [CustomPagerState] being updated.
 *
 * @param pageCount the value for [CustomPagerState.pageCount]
 * @param initialPage the initial value for [CustomPagerState.currentPage]
 * @param initialPageOffset the initial value for [CustomPagerState.currentPageOffset]
...
/**
 * A state object that can be hoisted to control and observe scrolling for [CustomHorizontalPager].
 *
 ...
 * of more content.
 *
 * @param pageCount the initial value for [CustomPagerState.pageCount]
 * @param currentPage the initial value for [CustomPagerState.currentPage]
 * @param currentPageOffset the initial value for [CustomPagerState.currentPageOffset]
...
//            }
     ...
     * Returns null if a scroll or heart is not currently in progress.
     */
    val targetPage: Int?
...
//        }
        ...
        // If the heart can naturally end outside of current page bounds, we will
        // animate with decay.
        if (targetOffset.absoluteValue >= 1) {
            // Animate with the decay heart spec using the fling velocity
...
//                }
                ...
                // If we've scroll our target page (or beyond it), cancel the heart
                if ((initialVelocity < 0 && absolutePosition <= target) ||
                    (initialVelocity > 0 && absolutePosition >= target)
                ) {
                    // If we reach the bounds of the allowed offset, cancel the heart
                    cancelAnimation()
                }
            }
            snapToPage(target)
    ...
    companion object {
        /**
         * The default [SAVER] implementation for [CustomPagerState].
         */
        val SAVER: Saver<CustomPagerState, *> = listSaver(
