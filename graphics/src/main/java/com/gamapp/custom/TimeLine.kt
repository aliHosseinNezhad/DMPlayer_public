...
fun TimeLine(
    ...
    val currentValueRange by rememberUpdatedState(newValue = valueRange)
    ...
    LayoutWithConstraints(
        ...
        val coerced by remember {
            derivedStateOf { value().coerceIn(valueRange.start, valueRange.endInclusive) }
        }
...
/**
 * Represents the colors used by a [TimeLine] and its parts in different states
 *
...
interface SliderColors {
     ...
     *
     * @param enabled whether the [TimeLine] is enabled or not
     */
    @Composable
...
private fun SliderImpl(
    ...
    Box(modifier.then(DefaultSliderConstraints)) {
        ...
        val offset by remember(thumbSize, widthDp) {
            derivedStateOf {
                (widthDp - thumbSize) * positionFraction()
            }
        }
...
private fun SliderThumb(
    ...
    Box(modifier.padding(start = offset().coerceAtLeast(1.dp))) {
        ...
        val elevation by remember {
            derivedStateOf {
                val value = if (interactions.isNotEmpty()) {
                    ThumbPressedElevation
                } else {
                    ThumbDefaultElevation
                }
                if (enabled) value else 0.dp
            }
        }
        Spacer(
            Modifier
                ...
                .graphicsLayer {
                    ...
                    clip = false
                    shape = CircleShape
                    this.ambientShadowColor = DefaultShadowColor
                    this.spotShadowColor = DefaultShadowColor
...
private fun Track(
    ...
    fun toTime(milis: Int): String {
        return String.format(
            "%02d:%02d",
            TimeUnit.MILLISECONDS.toMinutes(milis.toLong()) -
                    TimeUnit.HOURS.toMinutes(TimeUnit.MILLISECONDS.toHours(milis.toLong())),
            TimeUnit.MILLISECONDS.toSeconds(milis.toLong()) -
                    TimeUnit.MINUTES.toSeconds(TimeUnit.MILLISECONDS.toMinutes(milis.toLong()))
        );
    }
...
private val ThumbRippleRadius = 18.dp
