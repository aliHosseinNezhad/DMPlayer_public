...
import com.gamapp.SwipeableDefaults.resistanceConfig
...
const val TAG = "SwipeableTESTTAG"

/**
 * State of the [dmpSwipeable] modifier.
 *
 * This contains necessary information about any ongoing swipe or animation and provides methods
 * to change the state either immediately or by starting an animation. To create and remember a
 * [DmpSwipeableState] with the default animation clock, use [rememberSwipeableState].
 *
 ...
 */
@Stable
open class DmpSwipeableState<T>(
    initialValue: T,
    internal val animationSpec: AnimationSpec<Float> = tween(5000),
     ...
     * [dmpSwipeable] is currently settled. If a swipe or animation is in progress, this corresponds
     * the last anchor at which the [dmpSwipeable] was settled before the swipe or animation started.
     */
    ...
    /**
     * The current position (in pixels) of the [dmpSwipeable].
     *
    ...
    /**
     * The amount by which the [dmpSwipeable] has been swiped past its bounds.
     */
...
//        Log.i(TAG, "animateInternalToOffset: onDrag delta ${counter++}")
        flingJob.stop()
        dragBy(it)
    }


    private fun dragBy(it: Float) {
    ...
    private suspend fun animateInternalToOffset(target: Float, spec: AnimationSpec<Float>) {
        coroutineScope {
//            Log.i(TAG, "animateInternalToOffset: that drag $target **** ")
            ...
            try {
                flingJob.stop()
                flingJob = launch {
                    Animatable(prevValue).animateTo(target, tween(500)) {
     ...
     *
     * If a swipe is in progress, this is the value that the [dmpSwipeable] would animate to if the
    ...
    /**
     * The direction in which the [dmpSwipeable] is moving, relative to the current [currentValue].
     *
...
//                Log.i(TAG, "animateTo: currentValue $endValue")
            }
        }
    }
     ...
     * [velocity]. Fling with settling [dmpSwipeable] will always consume all the velocity provided
     * since it will settle at the anchor.
     *
     * In general cases, [dmpSwipeable] flings by itself when being swiped. This method is to be
     * used for nested scroll logic that wraps the [dmpSwipeable]. In nested scroll developer may
    ...
    suspend fun performFling(velocity: Float) {
        coroutineScope {
            ...
            animationJob = launch {
                latestNonEmptyAnchorsFlow.collect { anchors ->
//                    Log.i(TAG, "performFling: $anchors")
    ...
    /**
     * Force [dmpSwipeable] to consume drag delta provided from outside of the regular [dmpSwipeable]
     * gesture flow.
     *
     ...
     *
     * In general cases, [dmpSwipeable] drags by itself when being swiped. This method is to be
     * used for nested scroll logic that wraps the [dmpSwipeable]. In nested scroll developer may
    ...
    companion object {
        /**
         * The default [Saver] implementation for [DmpSwipeableState].
         */
        fun <T : Any> Saver(
            animationSpec: AnimationSpec<Float>,
            confirmStateChange: (T) -> Boolean
        ) = Saver<DmpSwipeableState<T>, T>(
            save = { it.currentValue },
            restore = { DmpSwipeableState(it, animationSpec, confirmStateChange) }
        )
    }
}

/**
 * Collects information about the ongoing swipe or animation in [dmpSwipeable].
 *
 * To access this information, use [DmpSwipeableState.progress].
 *
...
/**
 * Create and [remember] a [DmpSwipeableState] with the default animation clock.
 *
 * @param initialValue The initial value of the state.
 * @param animationSpec The default animation that will be used to animate to a new state.
 * @param confirmStateChange Optional callback invoked to confirm or veto a pending state change.
...
/**
 * Create and [remember] a [DmpSwipeableState] which is kept in sync with another state, i.e.:
 *  1. Whenever the [value] changes, the [DmpSwipeableState] will be animated to that new value.
 *  2. Whenever the value of the [DmpSwipeableState] changes (e.g. after a swipe), the owner of the
 *  [value] will be notified to update their state to the new value of the [DmpSwipeableState] by
 *  invoking [onValueChange]. If the owner does not update their state to the provided value for
 *  some reason, then the [DmpSwipeableState] will perform a rollback to the previous, correct value.
 */
@Composable
 ...
 *
 * When a swipe is detected, the offset of the [DmpSwipeableState] will be updated with the swipe
 * delta. You should use this offset to move your content accordingly (see `Modifier.offsetPx`).
 * When the swipe ends, the offset will be animated to one of the anchors and when that anchor is
 * reached, the value of the [DmpSwipeableState] will also be updated to the state corresponding to
 ...
 *
 * For an example of a [dmpSwipeable] with three states, see:
 *
 * @sample androidx.compose.material.samples.SwipeableSample
 *
 * @param T The type of the state.
 * @param state The state of the [dmpSwipeable].
 ...
 * @param orientation The orientation in which the [dmpSwipeable] can be swiped.
 * @param enabled Whether this [dmpSwipeable] is enabled and should react to the user's input.
 ...
 */
fun <T> Modifier.dmpSwipeable(
    state: DmpSwipeableState<T>,
    anchors: Map<Float, T>,
    ...
    Modifier.draggable(
        ...
        onDragStopped = { velocity ->
            launch {
                Log.i(TAG, "dmpSwipeable: $velocity")
...
/**
 * Interface to compute a threshold between two anchors/states in a [dmpSwipeable].
 *
...
/**
 * Specifies how resistance is calculated in [dmpSwipeable].
 *
 ...
 *
 * The resistance basis is usually either the size of the component which [dmpSwipeable] is applied
...
/**
 * Contains useful defaults for [dmpSwipeable] and [DmpSwipeableState].
 */
object SwipeableDefaults {
    /**
     * The default animation used by [DmpSwipeableState].
     */
    val AnimationSpec = SpringSpec<Float>()

    /**
     * The default velocity threshold (1.8 dp per millisecond) used by [dmpSwipeable].
     */
    ...
    /**
     * The default resistance config used by [dmpSwipeable].
     *
...
//internal val <T> DmpSwipeableState<T>.PreUpPostDownNestedScrollConnection: NestedScrollConnection
